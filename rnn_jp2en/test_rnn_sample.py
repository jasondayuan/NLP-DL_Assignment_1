...
def data_proc(data, device, config=RNNConfig()):
    '''
    ...
    for idx, jp_seq in enumerate(data):
...
if __name__ == "__main__":
    ...
    # Samples
    small_sample = ["私が黙っていたので彼女は余計に腹をたててしまった。"]
    ...
    # Results of given samples
    samples = []
    for text in small_sample:
        seq = [SOS_TOKEN]
        tokens = tokenizer_jp(text)
        for token in tokens:
            if token in jp_vocab_dict:
                seq.append(jp_vocab_dict[token])
            else:
                seq.append(jp_vocab_dict['<UNK>'])
        seq.append(EOS_TOKEN)
        samples.append(seq)
    
    samples = data_proc(samples, device)
    hypotheses = sampling(samples, encoder, decoder)
    detokenizer = TreebankWordDetokenizer()
    
    for hyp in hypotheses:
        sentence = detokenizer.detokenize(hyp)
        print(sentence)
